...
/// Lifted from Zed's `crates/util/src/util.rs` (Apache-2.0), whose only
/// caller here is this file. `design/rope-modifications.md` section 4.
#[inline]
const fn is_utf8_char_boundary(u8: u8) -> bool {
...
mod tests {
    ...
    fn test_random_chunks() {
        seeded(100, test_random_chunks_inner);
    }
    fn test_random_chunks_inner(mut rng: StdRng) {
    ...
    #[test]
    fn test_split_chunk_slice() {
        seeded(100, test_split_chunk_slice_inner);
    }
    fn test_split_chunk_slice_inner(mut rng: StdRng) {
        let text = &random_string_with_utf8_len(&mut rng, MAX_BASE);
        let chunk = Chunk::new(text);
    ...
    #[test]
    fn test_nth_set_bit_random() {
        seeded(1000, test_nth_set_bit_random_inner);
    }
    fn test_nth_set_bit_random_inner(mut rng: StdRng) {
    ...
    #[test]
    fn test_append_random_strings() {
        seeded(1000, test_append_random_strings_inner);
    }
    fn test_append_random_strings_inner(mut rng: StdRng) {
    ...
    #[test]
    fn test_prepend_random_strings() {
        seeded(1000, test_prepend_random_strings_inner);
    }
    fn test_prepend_random_strings_inner(mut rng: StdRng) {
        let len1 = rng.random_range(0..=MAX_BASE);
        let len2 = rng.random_range(0..=MAX_BASE).saturating_sub(len1);
        let str1 = random_string_with_utf8_len(&mut rng, len1);
        let str2 = random_string_with_utf8_len(&mut rng, len2);
