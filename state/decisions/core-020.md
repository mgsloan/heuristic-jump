...
Raised as `core-018` and renumbered here, because another worker's campaign
wrote a different question under that number in the same hour.

Half of it already has an answer in force from what that campaign landed:
`harness/capture-editor-traffic` and `harness/capture_editor_traffic.py` are in
the tree, so the location is `harness/` and the first option below is what
happened. What is left is narrower and still worth answering. That tool
captures the client half by teeing a real server behind an editor a human is
typing into; this campaign's route is Emacs under `--batch`, which needs no
human and so can be run by a loop. The question is whether the headless driver
joins it there.
